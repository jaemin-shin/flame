...
def generate_mid_agg_config(filename, cluster_num, cluster_value):
    # load json data
    ...
    data["channels"][0]["backend"] = "mqtt"
    data["channels"][1]["backend"] = "p2p"
    data["channels"][1]["groupBy"]["value"] = cluster_value
    data["hyperparameters"]["rounds"] = rounds
...
def generate_trainer_config(filename, num, cluster_num, cluster_value):
    ...
    data = json.load(file)
    data["job"]["id"] = job_id
    data["taskid"] = f"{str(task_id+num+(cluster_num+1)*10000)}"
    data["groupAssociation"]["param-channel"] = "cluster_" + str(cluster_num)
...
for i in range(total_cluster_num):
    filename = f"config/cluster{i}_mid_aggregator.json"
    generate_mid_agg_config(filename, i, cluster_value)

    os.chdir("middle_aggregator")
    os.system(
        f"python main.py ../config/cluster{i}_mid_aggregator.json > "
        + f"../output/middle_aggregator/cluster{i}_mid_aggregator.log 2>&1 &"
    )
    os.chdir("..")
